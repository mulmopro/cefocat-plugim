...
from subprocess import Popen, PIPE    # launch external programs
...
class run_plugin(object):
    ...
    def copy_exe(self):
        ...
        pOut = self.pathOUT / self.fExe
        
        try:
            sh.copyfile(pIn, pOut)
            print(self.fExe,'copied into', pOut.parent)

        except OSError as e:
            os.mkdir(self.pathOUT)
            print('copying',self.fExe,' into', pOut.parent)
            sh.copyfile(pIn, pOut)
    ...
    def edit_xml(self):
        ...
        pOut = self.pathOUT / self.fXml
        
        print('Reading the', self.fXml,' file')
